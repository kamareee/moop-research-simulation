...
def run_comparison_sim():
    ...
    for N, F, M in scenarios:
        ...
        # Storage for fulfillment stats across populations
        ...
        for i, pop in enumerate(pop_sizes):
            ...
            # 2. Knee Point Extraction for THIS population
            decomp = ASF()
            ...
            # Calculate and store percentages for stacked graph
            d_pct = (sim_res["e_depot"] / data["total_trip_demand"]) * 100
            l2_pct = (sim_res["e_L2"] / data["total_trip_demand"]) * 100
            dc_pct = (sim_res["e_DC"] / data["total_trip_demand"]) * 100
        ...
        # 3. Fulfillment Chart (Stacked per Population Size)
        fig_f, ax_f = plt.subplots(figsize=(4.0, 3.0))

        x_indices = np.arange(len(pop_sizes))
        depot_vals = [s["depot"] for s in pop_fulfillment_stats]
        l2_vals = [s["L2"] for s in pop_fulfillment_stats]
        dc_vals = [s["DC"] for s in pop_fulfillment_stats]

        # Stack the bars: Depot at bottom, then L2, then DC
        ax_f.bar(x_indices, depot_vals, color="#0077b6", label="Depot")
        ax_f.bar(
            x_indices, l2_vals, bottom=depot_vals, color="#00b4d8", label="Public L2"
        )
        ax_f.bar(
            x_indices,
            dc_vals,
            bottom=np.array(depot_vals) + np.array(l2_vals),
            color="#90e0ef",
            label="Public DC",
        )

        # Add percentage labels for total met demand [cite: 173]
        for idx, s in enumerate(pop_fulfillment_stats):
            total_met = s["depot"] + s["L2"] + s["DC"]
            ax_f.text(
                idx,
                total_met / 2,
                f"{int(total_met)}%",
