...
def get_advanced_case_data(N, F_count, M, alpha):
    ...
    # Depot: Constant 0.20/kWh
    pi_depot = np.full(T, 0.20)
        ...
        stations.append(
            {
                ...
                "power": 7.0,  # Level-2 Public Power
    ...
    for _ in range(N):
        ...
        # Only L2 detours are required now [cite: 68]
...
class ChargingProblem(ElementwiseProblem):
    def __init__(self, data):
        ...
        # Variable space: 0 (Skip), 1 (Depot), 2 to F+1 (L2 Stations)
...
def run_comparison_sim():
    scenarios = [[45, 10, 15], [60, 15, 20], [90, 20, 20]]
    ...
    for N, F, M in scenarios:
        ...
        # 3. Stacked Fulfillment Comparison (Depot & L2 Only)
        ...
        ax_f.legend(loc="upper center", bbox_to_anchor=(0.5, -0.15), ncol=2)
        fig_f.tight_layout()
