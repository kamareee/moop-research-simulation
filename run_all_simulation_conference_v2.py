...
def get_advanced_case_data(N, F_count, M, alpha):
    ...
    # 1. New Pricing Logic: Depot uses a constant 0.20
    pi_depot = np.full(T, 0.20)

    # 2. Public TOU Profile (Moved from depot to public L2) [cite: 68]
    pi_tou = np.zeros(T)
    ...
    for f in range(F_count):
        stations.append(
            {
                # L2 follows TOU; DC uses a unique rate in [0.40-0.50]
                "pi_pub": {"L2": pi_tou, "DC": round(random.uniform(0.40, 0.50), 3)},
    ...
    return {
        ...
        "F": F_count,
        "dt": dt,
        "pi_grid": pi_depot,  # Modified to constant
        "p_pv": p_pv,
        "p_base": p_base,
        "stations": stations,
        "fleet": fleet,
            ...
            for j, dwell in enumerate(v["dwells"]):
                ...
                if choice == 1:  # Depot
                    ...
                    if net_min > 0 and is_available:
                        ...
                        if c_type == "L2":
                            unit_price = st["pi_pub"]["L2"][start]
                            e_pub_L2 += energy_gained
                        else:
                            unit_price = st["pi_pub"]["DC"]
